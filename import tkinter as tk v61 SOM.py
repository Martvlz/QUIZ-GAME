...
class JogoDePerguntas:
    ...
    def setup_frames(self):
        self.question_frame = Toplevel(self.master)
        ...
        self.pontuacao_label = tk.Label(self.question_frame, text="Pontuação: 0", bg="white", font=("Arial", 12))
    ...
    def load_questions(self):
        self.fases = [
            ...
            {"titulo": "Fase 6: Sistemas Operacionais", "perguntas": self.perguntas_fase6()},
            {"titulo": "Fase 7: Hardware", "perguntas": self.perguntas_fase7()},
            {"titulo": "Fase 8: Internet das Coisas", "perguntas": self.perguntas_fase8()},
            {"titulo": "Fase 9: Programação Web", "perguntas": self.perguntas_fase9()},
            {"titulo": "Fase 10: Cloud Computing", "perguntas": self.perguntas_fase10()},
        ]
    ...
    def perguntas_fase6(self):
        return [
            {"pergunta": "O que é um sistema operacional?", "opcoes": ["A) Software que gerencia hardware", "B) Tipo de hardware", "C) Protocolo de rede", "D) Aplicativo"], "resposta": "A"},
            {"pergunta": "Qual sistema operacional é conhecido por sua segurança?", "opcoes": ["A) Windows", "B) Linux", "C) MacOS", "D) Android"], "resposta": "B"},
        ]

    def perguntas_fase7(self):
        return [
            {"pergunta": "O que é CPU?", "opcoes": ["A) Unidade de Processamento Central", "B) Tipo de memória", "C) Dispositivo de armazenamento", "D) Sistema operacional"], "resposta": "A"},
            {"pergunta": "Qual é a função da RAM?", "opcoes": ["A) Armazenar dados permanentemente", "B) Executar programas", "C) Armazenar dados temporariamente", "D) Controlar dispositivos"], "resposta": "C"},
        ]

    def perguntas_fase8(self):
        return [
            {"pergunta": "O que significa IoT?", "opcoes": ["A) Internet of Things", "B) Internet on Technology", "C) Interconnected Operations Technology", "D) Integrated Online Technology"], "resposta": "A"},
            {"pergunta": "Qual é um exemplo de dispositivo IoT?", "opcoes": ["A) Laptop", "B) Smartphone", "C) Smartwatch", "D) Desktop"], "resposta": "C"},
        ]

    def perguntas_fase9(self):
        return [
            {"pergunta": "Qual linguagem é usada para estilizar páginas web?", "opcoes": ["A) HTML", "B) CSS", "C) JavaScript", "D) PHP"], "resposta": "B"},
            {"pergunta": "O que é um servidor web?", "opcoes": ["A) Software", "B) Computador que armazena sites", "C) Dispositivo de entrada", "D) Protocolo"], "resposta": "B"},
        ]

    def perguntas_fase10(self):
        return [
            {"pergunta": "O que é Cloud Computing?", "opcoes": ["A) Computação em nuvem", "B) Armazenamento em disco rígido", "C) Software local", "D) Tipo de hardware"], "resposta": "A"},
            {"pergunta": "Qual é uma vantagem da computação em nuvem?", "opcoes": ["A) Acesso limitado", "B) Redução de custos", "C) Necessidade de hardware poderoso", "D) Dificuldade de acesso"], "resposta": "B"},
        ]
    ...
    def iniciar_jogo(self, nome):
        ...
        self.entry_frame.pack_forget()  # Remove o frame de entrada
        self.iniciar_fase()
...
if __name__ == "__main__":
    ...
    tk.Button(entry_frame, text="Iniciar Jogo", command=lambda: jogo.iniciar_jogo(nome_entry.get()), bg="#4CAF50", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
